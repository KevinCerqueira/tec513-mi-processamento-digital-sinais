...
fp = 4500;                  % Freq Passagem
fs = 4000;                  % Freq de Rejeição
...
for i = 1:length(windows_const)
    ...
    % filtro ideal passa-alta (invertendo o filtro passa-baixa)
    n = -(M-1)/2:(M-1)/2;
    % Passa-alta
    ...
    audiowrite(sprintf('audio_filtrado_PA_%s.wav', janela_names{i}), audio_filtrado, FreqAmostragem);
